...
function Quiz(props) {
    ...
    return (
        <div className="posts-quiz-frontend" style={{ backgroundColor: props.bgColor, textAlign: props.theAlign }}>
            <p>{props.question}</p>
            <ul>
                {props.answers.map((answer) => {
                    return (
                        <li className={
                            (isCorrectDelayed === true && answer.id == props.correctAnswer ? "no-click" : "") + (isCorrectDelayed === true && answer.id != props.correctAnswer ? "fade-incorrect" : "")
                        } onClick={() => {
                            isCorrect === true ? undefined : handleAnswer(answer.id)
                        }}>
                            {isCorrectDelayed === true && answer.id == props.correctAnswer && (
